        ...
        @else
            <div class="space-y-3">
                @foreach($requirements as $i => $req)
                    <div class="kp-req-item {{ count($req->sub_items ?? []) > 0 ? 'has-sub' : '' }}">
                        <div class="kp-req-num">{{ $i + 1 }}</div>
                        <div class="flex-1">
                            <p class="text-gray-700 font-medium text-sm">{{ $req->text }}</p>
                            @if($req->note)
                                <span class="kp-req-note">{{ $req->note }}</span>
                            @endif
                            @if(!empty($req->sub_items))
                                <div class="kp-req-sub">
                                    @foreach($req->sub_items as $sub)
                                        <div class="kp-req-sub-item">{{ $sub }}</div>
                                    @endforeach
                                </div>
                            @endif
                        </div>
            ...
            <div class="space-y-3">
                @foreach($jadwal as $j)
                    <div class="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-4 border border-green-100">
                        <p class="font-bold text-gray-800">{{ $j->periode }}</p>
                        @if($j->tanggal)
                            <p class="text-sm text-gray-600 mt-1">📅 {{ $j->tanggal }}</p>
                        @endif
                        @if($j->keterangan)
                            <p class="text-sm text-gray-500 mt-1">{{ $j->keterangan }}</p>
                        @endif
                        @if($j->pdf_url)
                            <a href="{{ $j->pdf_url }}" target="_blank"
...
async function loadKPRecap() {
    ...
    const res   = await fetch(`{{ route('api.kp-data') }}?month=${month}`);
