...
<head>
    ...
    <title>@yield('title', 'Beranda') — KLIK-SDM | Kemudahan Layanan Informasi Kepegawaian</title>
...
<header class="gradient-bps text-white shadow-xl sticky top-0 z-50">
    <div class="px-4 py-3">
        <div class="flex items-center justify-between">
            <div class="flex items-center space-x-3">
                ...
                <div class="flex items-center space-x-3">
                    <img src="{{ asset('assets/images/logo_bps.png') }}" alt="BPS" class="h-10 w-10 object-contain">
                    <div>
            ...
            <div class="flex items-center space-x-2">
                @auth
                    <div class="hidden md:flex items-center space-x-2 bg-white/10 rounded-xl px-3 py-2">
                        ...
                        <div class="text-sm">
                            ...
                            <p class="text-blue-200 text-xs">{{ auth()->user()->role_label }}</p>
                        </div>
                    </div>
                    <a href="{{ route('admin.dashboard') }}" class="p-2 rounded-lg hover:bg-white/10 transition-colors text-xs font-semibold px-3">Admin</a>
                ...
                @else
                    <a href="{{ route('login') }}" class="p-2 rounded-lg hover:bg-white/10 transition-colors text-sm font-semibold px-3">Login Admin</a>
                @endauth
            </div>
        </div>
    </div>
    ...
    <div class="bg-gradient-to-r from-yellow-400 to-orange-400 py-2 scrolling-text shadow-md">
        <span class="text-gray-800 font-semibold text-xs">📢 {{ $scrollingText ?? 'Selamat Datang di KLIK-SDM BPS Provinsi Sulawesi Utara' }}</span>
    </div>
</header>
