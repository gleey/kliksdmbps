...
<head>
    ...
    <title>Admin — @yield('title', 'Dashboard') | KLIK-SDM</title>
...
<header class="gradient-bps text-white shadow-xl sticky top-0 z-50">
    <div class="px-6 py-3 flex items-center justify-between">
        <div class="flex items-center space-x-3">
            <img src="{{ asset('assets/images/logo_bps.png') }}" alt="BPS" class="h-9 w-9 object-contain">
            <div>
        ...
        <div class="flex items-center space-x-3">
            <a href="{{ route('home') }}" class="text-blue-200 hover:text-white text-sm transition-colors">← Lihat Publik</a>
            <div class="flex items-center space-x-2 bg-white/10 rounded-xl px-3 py-2">
                ...
                <span class="admin-badge {{ auth()->user()->role === 'admin_pengelola' ? 'pengelola' : 'kepegawaian' }} ml-1">
    ...
    <aside class="w-60 bg-white shadow-md min-h-screen">
        <nav class="p-4 space-y-1">
            <a href="{{ route('admin.dashboard') }}"
               class="sidebar-item {{ request()->routeIs('admin.dashboard') ? 'active' : '' }} flex items-center gap-3 p-3 rounded-lg text-gray-700 text-sm">
                🏠 <span>Dashboard</span>
            </a>
            <div class="pt-2 pb-1 px-3 text-xs font-bold text-gray-400 uppercase tracking-wider">Konten</div>
            <a href="{{ route('admin.pengumuman.index') }}"
               class="sidebar-item {{ request()->routeIs('admin.pengumuman.*') ? 'active' : '' }} flex items-center gap-3 p-3 rounded-lg text-gray-700 text-sm">
                📢 <span>Pengumuman</span>
            </a>
            <a href="{{ route('admin.gallery.index') }}"
               class="sidebar-item {{ request()->routeIs('admin.gallery.*') ? 'active' : '' }} flex items-center gap-3 p-3 rounded-lg text-gray-700 text-sm">
                🖼️ <span>Galeri SDM</span>
            </a>
            <a href="{{ route('admin.tim-kerja.index') }}"
               class="sidebar-item {{ request()->routeIs('admin.tim-kerja.*') ? 'active' : '' }} flex items-center gap-3 p-3 rounded-lg text-gray-700 text-sm">
                👥 <span>Tim SDM & Hukum</span>
            </a>
            <a href="{{ route('admin.faq.index') }}"
               class="sidebar-item {{ request()->routeIs('admin.faq.*') ? 'active' : '' }} flex items-center gap-3 p-3 rounded-lg text-gray-700 text-sm">
                ❓ <span>FAQ</span>
            </a>
            ...
            <a href="{{ route('admin.kp.index') }}"
               class="sidebar-item {{ request()->routeIs('admin.kp.*') ? 'active' : '' }} flex items-center gap-3 p-3 rounded-lg text-gray-700 text-sm">
                📈 <span>Kenaikan Pangkat</span>
            </a>
            <a href="{{ route('admin.kgb.index') }}"
               class="sidebar-item {{ request()->routeIs('admin.kgb.*') ? 'active' : '' }} flex items-center gap-3 p-3 rounded-lg text-gray-700 text-sm">
                💰 <span>Kenaikan Gaji Berkala</span>
            </a>
            <a href="{{ route('admin.uji-kompetensi.index') }}"
               class="sidebar-item {{ request()->routeIs('admin.uji-kompetensi.*') ? 'active' : '' }} flex items-center gap-3 p-3 rounded-lg text-gray-700 text-sm">
                📋 <span>Uji Kompetensi</span>
            </a>
            <a href="{{ route('admin.peraturan.index') }}"
               class="sidebar-item {{ request()->routeIs('admin.peraturan.*') ? 'active' : '' }} flex items-center gap-3 p-3 rounded-lg text-gray-700 text-sm">
                📜 <span>Peraturan</span>
            </a>
            <div class="pt-2 pb-1 px-3 text-xs font-bold text-gray-400 uppercase tracking-wider">Layanan</div>
            @foreach([['tugas-belajar','🎓','Tugas Belajar'],['karis-karsu','💳','KARIS/KARSU'],['perkawinan-pertama','💍','Perkawinan Pertama'],['pensiun','🏖️','Pensiun']] as [$slug,$icon,$label])
            <a href="{{ route('admin.layanan.index', $slug) }}"
               class="sidebar-item {{ request()->is('admin/layanan/'.$slug.'*') ? 'active' : '' }} flex items-center gap-3 p-3 rounded-lg text-gray-700 text-sm">
            ...
            <a href="{{ route('admin.settings.index') }}"
               class="sidebar-item {{ request()->routeIs('admin.settings.*') ? 'active' : '' }} flex items-center gap-3 p-3 rounded-lg text-gray-700 text-sm">
